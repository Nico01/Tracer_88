...
static void OUTWRITE(int p, char *b, long n) /* part number, buffer pointer, write count */
{
...
static void BEGINSEEK(int p, long o) /* part number, offset in file */
{
    struct fil *ptr = &__parts[p];
...
#endif
    if (p >= PARTRELO)
        o = 0; /* no attempt to align writes for the time being */
...
void wr_ohead(struct outhead *head)
{
    ...
    if (BYTE_ORDER != 0x0123 || sizeof(struct outhead) != SZ_HEAD) {
        ...
        put2(head->oh_magic, c);
        c += 2;
        put2(head->oh_stamp, c);
        c += 2;
        put2(head->oh_flags, c);
        c += 2;
        put2(head->oh_nsect, c);
        c += 2;
        put2(head->oh_nrelo, c);
        c += 2;
        put2(head->oh_nname, c);
        c += 2;
        put4(head->oh_nemit, c);
        c += 4;
