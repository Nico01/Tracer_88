...
int main(int argc, char *argv[])
{
    ...
    case 1:
        break;
    case (S_ETC | S_COM | S_VAR | S_DOT) != S_ETC:
        break;
    }
    ...
    p = *argv++;
    if (!(argc--)) {
        fprintf(stderr, "No argument? %s\n", p);
        exit(1);
    }
    sprintf(ffnm, "%s.s", p);
    sprintf(filsnm, "%s.s", projname);
    sprintf(filcapnm, "%s.S", projname);
    if (((inptr = fopen(p, "r")) == NULL) && ((inptr = fopen(ffnm, "r")) == NULL) &&
        ((inptr = fopen(filsnm, "r")) == NULL) && ((inptr = fopen(filcapnm, "r")) == NULL))
    ...
    while (argc-- > 0) {
        ...
        if (((inptr = fopen(p, "r")) == NULL) && ((inptr = fopen(filsnm, "r")) == NULL) &&
...
#endif */
#ifdef ASLD
        if (fread(armagic, 2, 1, input) == 1 &&
            ((armagic[0] & 0377) | ((unsigned)(armagic[1] & 0377) << 8)) == ARMAG) {
            archive();
            fclose(input);
            /* continue;*/
        }
...
void archive(void) {
    ...
    for (;;) {
        ...
        header.ar_size =
            (((((long)(getsize[AR_SIZE + 1] & 0377)) << 8) + ((long)(getsize[AR_SIZE] & 0377))
              << 8) +
                 ((long)(getsize[AR_SIZE + 3] & 0377))
             << 8) +
...
void commfinish(void) {
    ...
    for (cp = commons; cp; cp = cp->c_next) {
        ...
        if (!(ip->i_type & S_EXT)) {
            ...
            if ((sflag & SYM_EXT) && PASS_SYMB)
                newsymb(ip->i_name, ip->i_type & (S_EXT | S_TYP), 0, load(ip));
#else /* not ASLD */
#ifdef THREE_PASS
        if (pass == PASS_2) {
            cp->c_size -= sp->s_gain;
    ...
    if (PASS_SYMB) {
        ...
        newsymb(ip->i_name, ip->i_type, 0, cp->c_size);
    }
#endif /* not ASLD */
        }
        if (PASS_SYMB == 0)
        ...
        for (i = 0; i < H_SIZE; i++)
            for (ip = hashtab[H_LOCAL + i]; ip; ip = ip->i_next) {
                ...
                if (pass != PASS_3)
                     ...
                     */
                    ip->i_valu = outhead.oh_nname;
                newsymb(ip->i_name, S_EXT | S_UND, 0, (valu_t)0);
            }
#endif  /* not ASLD */
        /*
        ...
        if (sflag & SYM_SCT)
            for (sp = sect; sp < &sect[outhead.oh_nsect]; sp++) {
                ip = sp->s_item;
                newsymb(ip->i_name, (ip->i_type | S_SCT), 0, load(ip));
            }
    }
